...
void AirSpace::Update()
{
  ...
  if (randomProp(engine) < missileGenProb && missiles.size() < maxMissiles)
  {
    if (randomProp(engine) < smartMissileGenProb) {
      missiles.emplace_back(std::make_unique<SmartMissile>(random_w(engine), random_w(engine)));
    } else{
...
void SmartMissile::React(std::vector<std::unique_ptr<Missile>> &missiles)
{
  reactTimer--;
  if (reactTimer <= 0)
  {
